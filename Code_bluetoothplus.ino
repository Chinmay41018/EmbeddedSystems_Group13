...
SoftwareSerial BTSerial(UNORX, UNOTX); //(RX,TX) CONNECT BT RX PIN TO ARDUINO 3 PIN | CONNECT BT TX PIN TO ARDUINO 2 PIN
//-----------------------end bluetooth interface buildup
...
void setup() {
  ...
  if (MAX_N_CHANNELS > 8) {
    Serial.begin(9600*2);  //Need 115200 for 16-channels, only need 115200 for 8-channels but let's do 115200*2 for consistency
  } else {
    Serial.begin(9600);
  }
  ...
  BTSerial.write(int(F("ADS1299-Arduino UNO - Stream Raw Data")));
  BTSerial.write("\r\n"); //read the string from Flash to save RAM
  BTSerial.write(int(F("Configured as OpenBCI_Version code = ")));
  BTSerial.write(OpenBCI_version);
  BTSerial.write(int(F(", isDaisy = ")));
  BTSerial.write(ADSManager.isDaisy);
  BTSerial.write(int(F("Configured for ")));
  BTSerial.write(MAX_N_CHANNELS);
  BTSerial.write(int(F(" Channels")));
  BTSerial.write("\r\n");
  ...
  if (useFilters)
  {
    Serial.print(F("Configured to do some filtering here on the Arduino."));
    BTSerial.write(int(F("Configured to do some filtering here on the Arduino.")));
  }
  ...
  BTSerial.write("Press '?' to query and print ADS1299 register settings again"); //read it straight from flash
  BTSerial.write("\r\n");
  BTSerial.write("Press 1-8 to disable EEG Channels, q-i to enable (all enabled by default)");
  BTSerial.write("\r\n");
  BTSerial.write("Press 'f' to enable filters.  'g' to disable filters");
  BTSerial.write("\r\n");
  BTSerial.write("Press 'x' (text) or 'b' (binary) to begin streaming data...");
  BTSerial.write("\r\n");
  ...
  if (is_running) {
    ...
    switch (outputType) {
      ...
      case OUTPUT_BINARY_OPENEEG_SYNTHETIC:
        ...
        break;
      default:
        astext = ADSManager.printChannelDataAsText(MAX_N_CHANNELS,sampleCounter);  //print all channels, whether active or not
//        Serial.print("JOSHI");
        Serial.println(astext);
        int str_len = astext.length();        
        char char_array[str_len],newchar[str_len];
//        astext.toCharArray(char_array, str_len);
        for (int i=0;i<str_len-1;i++)
        BTSerial.write(astext[i]);
  ...
  while(BTSerial.available()){
    ...
    switch (inChar)
    {
      ...
      case 'n':
        ...
        if (is_running)
        {
          Serial.println(F("Arduino: Starting binary (including AUX value)..."));
          BTSerial.write("Arduino: Starting binary (including AUX value)...");
          BTSerial.write("\r\n");
        }
        break;
      case 'b':
        ...
        if (is_running)
        {
          Serial.println(F("Arduino: Starting binary..."));
          BTSerial.write("Arduino: Starting binary...");
          BTSerial.write("\r\n");
        }
      ...
      case 'v':
        ...
        if (is_running)
        {
          Serial.println(F("Arduino: Starting binary 4-chan..."));
          BTSerial.write("Arduino: Starting binary 4-chan...");
          BTSerial.write("\r\n");
        }
        break;
     case 's':
     ...
     case 'x':
        ...
        if (is_running)
        {
          Serial.println(F("Arduino: Starting text..."));
//          BTSerial.write("Chinamy");
          BTSerial.write("Arduino: Starting text...");
          BTSerial.write("\r\n");
        }
        break;
     case 'f':
        ...
        BTSerial.write("Arduino: enabaling filters");
        BTSerial.write("\r\n");
        break;
     case 'g':
        useFilters = false;
        Serial.println(F("Arduino: disabling filters"));
        BTSerial.write("Arduino: disabling filters");
        BTSerial.write("\r\n");
        break;
     case '?':
        //print state of all registers
...
int changeChannelState_maintainRunningState(int chan, int start)
{
  ...
  if (start == true) {
    ...
    BTSerial.write("Activating channel ");
    BTSerial.write(chan);
    BTSerial.write("\r\n");
    ...
    BTSerial.write("Deactivating channel ");
    BTSerial.write(chan);
    BTSerial.write("\r\n");
...
int changeChannelLeadOffDetection_maintainRunningState(int chan, int start, int code_P_N_Both)
{
  ...
  if (start == true) {
    ...
    BTSerial.write("Activating channel ");
    BTSerial.write(chan);
    BTSerial.write(" Lead-Off Detection");
    BTSerial.write("\r\n");
