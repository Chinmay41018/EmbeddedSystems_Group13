...
SoftwareSerial BTSeriala(UNORX, UNOTX); //(RX,TX) CONNECT BT RX PIN TO ARDUINO 3 PIN | CONNECT BT TX PIN TO ARDUINO 2 PIN
...
String ADS1299Manager::printChannelDataAsText(int N, long int sampleNumber)
{
	//check the inputs
	String retval="";
	if ((N < 1) || (N > n_chan_all_boards)) return retval;

	//print the sample number, if not disabled
	if (sampleNumber > 0) {
    retval.concat(String(sampleNumber));
    		retval.concat(", ");
		// Serial.print(sampleNumber);
		// Serial.print(", ");
		//
		// BTSeriala.write(char(sampleNumber));
		// BTSeriala.write(", ");
	...
	{
		retval.concat(String(channelData[chan]));
		retval.concat(", ");
		// Serial.print(channelData[chan]);
		// Serial.print(", ");
		// BTSeriala.write(char(channelData[chan]));
		// BTSeriala.write(", ");
	}
return retval;
retval = "";
	...
	long int auxValue, boolean useSyntheticData)
{
  ...
  BTSeriala.write( (byte) PCKT_START);
  ...
  BTSeriala.write(val_ptr,4);
	...
	for (int chan = 0; chan < N; chan++ )
	{
		...
		BTSeriala.write(val_ptr,4); //4 bytes long
	}
